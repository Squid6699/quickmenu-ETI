...
routerLogin.post("/login", async (req, res) => {
    ...
    const query = "SELECT username, password, name, roleId FROM users WHERE username = ? AND BINARY password = ?";
    ...
    db.query(query, [username, password], (err, result) => {
        ...
        return res.status(200).json({success: true, msg: "LOGIN SUCCESSFUL", token: token, name: result[0].name, roleId: result[0].roleId});
    });
});
